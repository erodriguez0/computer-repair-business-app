 ...
 * @author esteban
 */
public class CompleteRequestRecordsController extends CSWindowFXMLController implements Initializable {
    ...
    @FXML Button applyBtn, resetBtn, refreshBtn, reportBtn, historyBtn;
...
//        emp_id = super.getEmpID();
        ...
        try {
            ...
            Logger.getLogger(CompleteRequestRecordsController.class.getName()).log(Level.SEVERE, null, ex);
        }
...
//                        + "/src/erodriguez6/db/CSDept_stats_report.jrxml");
...
//        });
        ...
        CSTable.setOnMouseClicked( event -> {
            if(event.getClickCount() == 2 && !CSTable.getSelectionModel().isEmpty() 
                    && CSTable.getSelectionModel().getSelectedItem().getStatus().equalsIgnoreCase("ACCEPTED")) {
...
//                        buildCSTable();
                ...
                try {
                    problemPane = FXMLLoader.load(getClass().getResource("EditExistingRecord.fxml"));
            ...
            } else if (event.getClickCount() == 2 && !CSTable.getSelectionModel().isEmpty() 
                    && CSTable.getSelectionModel().getSelectedItem().getStatus().equalsIgnoreCase("DECLINED")) {
                //Set problem_id of double clicked tableview row
                pID = CSTable.getSelectionModel().getSelectedItem().getProblemID();
//                problem_update(pID, emp_id);
                final Stage dialog = new Stage();
...
//                        buildCSTable();
                ...
                BorderPane problemPane;
                try {
                    problemPane = FXMLLoader.load(getClass().getResource("ProblemLookUp.fxml"));
                    Scene dialogScene = new Scene(problemPane);
                    dialog.setScene(dialogScene);
                    dialog.show();
                } catch (IOException ex) {
    ...
    private void buildCSTable(DatePicker b, DatePicker e) 
        ...
        try {
            ...
            if (b.getValue() == null && e.getValue() == null) {
                ...
                sql = "select * from csinfohistory where submit_date <= ? order by submit_date";
                cStmnt = conn.prepareCall(sql);
                ...
                sql = "select * from csinfohistory where submit_date >= ? and submit_date <= ? order by submit_date";
                cStmnt = conn.prepareCall(sql);
            ...
            while(rs.next()) {
                ...
                problem.setProblemID(rs.getInt("problem_id"));
                String str1;
                str1 = rs.getString("fname") + " " + rs.getString("lname");
                problem.setFname(str1);
//                problem.setLname(rs.getString("lname"));
                problem.setUsername(rs.getString("email"));
                if(rs.getDate("review_date") == null) {
